...
app.post('/webhook', verifyShopifyWebhook, async (req, res) => {
  try {
    ...
    const ventaDoc = {
      ...
      autorizacion: "",
      cliente: {
        cedula: safeString(order.customer?.id, "9999999999"),
        razon_social: safeString(`${order.customer?.first_name || "Cliente"} ${order.customer?.last_name || "Shopify"}`, "Cliente Shopify"),
        email: safeString(order.customer?.email, "noemail@example.com"),
        direccion: safeString(order.billing_address?.address1, "Sin dirección"),
        telefono: safeString(order.billing_address?.phone, "Sin teléfono"),
        tipo: "CLIENTE"
      },
      detalles: order.line_items.map(item => {
        // Validar y convertir valores numéricos de forma segura
        const precio = safeParseFloat(item.price, 0);
        const cantidad = safeParseInt(item.quantity, 1);
        ...
        return {
          producto_id: safeString(item.sku || item.product_id, "PROD-001"),
          cantidad: cantidad,
          precio: precio,
          porcentaje_iva: 12,
      ...
      cobros: [{
        ...
        monto: safeParseFloat(order.total_price, 0),
        numero_cheque: "",
        tipo_ping: ""
      }]
    };
